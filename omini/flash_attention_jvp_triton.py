...
# -----------------------------------------------------------------------------
# 1. Kernel Helper (保持不变)
# -----------------------------------------------------------------------------
# def _run_flash_attn_kernel(q, k, v, tq, tk, tv, sm_scale):
#     B, H = q.shape[:-2]
...
#         B, H, SEQ_LEN_Q, SEQ_LEN_KV, HEAD_DIM_QK, HEAD_DIM_V,
#         # # 请根据你的 Kernel 实现补充这些参数
...
# -----------------------------------------------------------------------------
# 2. Autograd Function (集成 Backward 和 JVP)
# -----------------------------------------------------------------------------
def _debug_check_tensor(name, x):
    """
    辅助调试函数：检查变量是否可以传入 Triton Kernel
    """
    ...
    if isinstance(x, torch.Tensor):
        ...
        # 1. 检查是否是 Meta (Fake) Tensor
        if x.device.type == 'meta':
            print("  !!! [FAIL] Tensor is on META device (No storage).")
            return False

        # 2. 尝试获取 storage
        try:
            has_storage = x.storage().size() > 0
            print(f"  Storage size: {x.storage().size()}")
        except Exception as e:
            print(f"  !!! [FAIL] Cannot access storage: {e}")
            # 注意：某些 FunctionalTensor 可能直接在这步报错
            return False

        # 3. 尝试获取 data_ptr (核心检查点)
        try:
            ptr = x.data_ptr()
            print(f"  Data Ptr: {ptr}")
            if ptr == 0 and x.numel() > 0:
                print("  !!! [FAIL] Data pointer is NULL (0) but tensor is not empty.")
                return False
        except RuntimeError as e:
            print(f"  !!! [FAIL] x.data_ptr() raised RuntimeError: {e}")
            return False

    # 4. 检查标量是否误传为 Tensor
    elif isinstance(x, (float, int)):
        print(f"  Value: {x} (OK - Primitive type)")
    else:
        print(f"  Value: {x}")
...
def _run_flash_attn_kernel(q, k, v, tq, tk, tv, sm_scale):
    #print("\n\n====== [DEBUG START] Checking arguments for Triton Kernel ======")

    # # 将所有要传给 kernel 的 Tensor 和关键变量放这里检查
    # check_list = [
    #     ('q', q), ('k', k), ('v', v),
    #     ('tq', tq), ('tk', tk), ('tv', tv),
    #     ('sm_scale', sm_scale)
    ...
    #
    # has_error = False
    # for name, var in check_list:
    #     if not _debug_check_tensor(name, var):
    #         print(f"  >>> PROBLEM DETECTED IN VARIABLE: {name}")
    #         has_error = True
    #
    # if has_error:
    #     # 如果你想强制停止看日志，可以取消下面这行的注释
    #     raise ValueError("Debug check failed. See logs above.")
    # # 1. 【关键修复】强制将 sm_scale 转为 python float
    # # 如果它是 Tensor (即使是标量)，Triton 会试图读取它的地址导致崩溃
    ...
    # 2. 分配输出
    # 使用 q.device 是安全的，只要 q.device 不是 'meta' (前面步骤已排除)
    o = torch.empty((B, H, SEQ_LEN_Q, HEAD_DIM_V), device=q.device, dtype=q.dtype)
    to = torch.empty_like(o)
    ...
    _attn_fwd[grid](
        q, k, v,
        tq, tk, tv,
        sm_scale,  # 这里现在是 float 了
        M, o, to,
        # 3. 【推荐修复】强制 stride 转为 python int，防止 Symbolic Integer 传入
        int(q.stride(0)), int(q.stride(1)), int(q.stride(2)), int(q.stride(3)),
        int(k.stride(0)), int(k.stride(1)), int(k.stride(2)), int(k.stride(3)),
        int(v.stride(0)), int(v.stride(1)), int(v.stride(2)), int(v.stride(3)),
        int(o.stride(0)), int(o.stride(1)), int(o.stride(2)), int(o.stride(3)),
        B, H, SEQ_LEN_Q, SEQ_LEN_KV, HEAD_DIM_QK, HEAD_DIM_V,
    )
    return o, to, M


class FlashAttnJVP(torch.autograd.Function):

    @staticmethod
    def forward(q, k, v, sm_scale=None):
        if sm_scale is None:
        ...
        o, _, M = _run_flash_attn_kernel(q, k, v, tq, tk, tv, sm_scale)
        return o, M
    ...
    def setup_context(ctx, inputs, outputs):
        ...
        # ============================================================
        # 【重要】使用自定义属性保存，不要用 save_for_backward
        # save_for_backward 只对 backward 有效，对 jvp 无效！
        # ============================================================
        ctx.q_primal = q
        ctx.k_primal = k
        ctx.v_primal = v
        ctx.sm_scale = float(sm_scale) if isinstance(sm_scale, torch.Tensor) else float(sm_scale)
    ...
    #         "Backward not supported. Use torch.func.jvp() instead."
    #     )

    @staticmethod
    def jvp(ctx, tq, tk, tv, t_sm_scale=None):
        # 获取 primal（这些是 TensorWrapper）
        q = ctx.q_primal
        k = ctx.k_primal
        v = ctx.v_primal
        ...
        # ============================================================
        # 【核心修复】使用 _DisableFuncTorch 禁用追踪
        # 在这个上下文中，所有 tensor 操作都会产生真正的物理 tensor
        # ============================================================
        with torch._C._DisableFuncTorch():
            # 现在 clone 会产生物理 tensor，不再是 wrapper
            q_in = q.detach().clone().contiguous()
            k_in = k.detach().clone().contiguous()
            v_in = v.detach().clone().contiguous()

            tq_in = tq.detach().clone().contiguous()
            tk_in = tk.detach().clone().contiguous()
            tv_in = tv.detach().clone().contiguous()

            # 验证（可选，调试用）
            #print(f"q_in data_ptr: {q_in.data_ptr()}")  # 应该成功

            # 调用 Triton Kernel
            _, to, M_unused = _run_flash_attn_kernel(
                q_in, k_in, v_in,
                tq_in, tk_in, tv_in,
                sm_scale
            )
...
# 3. 外部包装函数 (隐藏 M)
# -----------------------------------------------------------------------------
def custom_flash_attn_func(q, k, v, sm_scale=None):
    # 调用 apply，得到 (o, M)
    # print("custom_flash_attn_func")
    o, M = FlashAttnJVP.apply(q, k, v, sm_scale)
    # 我们只返回 o 给用户/模型
    return o
